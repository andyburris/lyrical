fun List<String>.randomLyricIndex(difficulty: Difficulty, title: String): Int {
    ...
    val selectedLyric = when (difficulty) {
        ...
        Difficulty.Hard -> byFrequency.filterNotSongTitle(title).filterWordCount(3).filterFrequency { it == 1 }
...
private fun Map<String, List<IndexedValue<String>>>.filterFrequency(block: (Int) -> Boolean) = this.filter { block(it.value.size) }.ifEmpty { this }
