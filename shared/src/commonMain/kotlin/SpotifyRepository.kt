...
sealed class SpotifyRepository() {
    data class LoggedIn(
        ...
        val onReauthenticate: (AuthAction.Authenticate) -> Unit,
    ) : SpotifyRepository() {
        internal val cachedPlaylists = mutableListOf<SimplePlaylist>()
        suspend fun getPlaylistTracks(playlistURI: String): List<Track> = handleReauth(emptyList()) {
        ...
        suspend fun getFeaturedPlaylists(): List<SimplePlaylist> = handleReauth(emptyList()) { api.browse.getFeaturedPlaylists().playlists.items.also { cachedPlaylists.addAll(it); cachedPlaylists.retainDistinct() } }
        suspend fun searchPlaylists(query: String): List<SimplePlaylist> = handleReauth(emptyList()) { api.search.search(query, SearchApi.SearchType.Playlist).playlists?.items ?: emptyList() }
